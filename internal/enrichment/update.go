...
func UpdateModelMetadataFile(registryModel string, enrichedData *types.EnrichedModelMetadata) error {
	...
	if enrichedData.Tags.Source == "huggingface" && enrichedData.Tags.Value != nil {
		...
		if ok {
			...
			if tagLicense != "" && existingMetadata.License == nil {
				...
				enrichmentInfo.DataSources.License = "huggingface"
				// Automatically set license link if we have a well-known license
	...
	if enrichedData.Tags.Source == "huggingface" && enrichedData.Tags.Value != nil {
		...
		if ok {
			...
			if len(languages) > 0 && len(existingMetadata.Language) == 0 {
				...
				enrichmentInfo.DataSources.Language = "huggingface"
			}
		}
	}

	// Handle tasks from enriched data first (highest priority)
	if enrichedData.Tasks.Source == "huggingface" && enrichedData.Tasks.Value != nil {
		...
		if ok && len(tasks) > 0 {
			...
			enrichmentInfo.DataSources.Tasks = "huggingface"
		}
	} else if enrichedData.Tags.Source == "huggingface" && enrichedData.Tags.Value != nil {
	...
	if len(existingMetadata.Tasks) == 0 && existingMetadata.Readme != nil {
		...
		if len(inferredTasks) > 0 {
			...
			enrichmentInfo.DataSources.Tasks = "readme"
		}
	}
	...
	if enrichedData.LastModified.Source == "huggingface" && enrichedData.LastModified.Value != nil {
